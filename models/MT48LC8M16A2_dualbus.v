...
module MT48LC8M16A2_dualbus #(parameter Debug=0) (data_rd, data_wr, addr, ba, clk, cke, csb, rasb, casb, web, dqm);

    parameter addr_bits =      13;
    parameter data_bits =      16;
    parameter col_bits  =       9;
    parameter mem_sizes = 2 ** (addr_bits+col_bits);                                  // 2 Meg
 
    output     [data_bits - 1 : 0] data_rd;
    ...
    assign    data_rd               = Dq_reg;                        // DQ buffer
    ...
    always @ (posedge Sys_clk) begin
        ...
        if (Read_enable == 1'b1 || Write_enable == 1'b1) begin
           ...
           //->tb.test_control.error_detected;
                $display("at time %t ERROR: Cannot Read or Write - Bank %d is not Activated (%d)", $time, ba, Pc_b0);
            end
         ...
         //->tb.test_control.error_detected;
            ...
            // Write Command
                ...
                if (addr[10] == 1'b1) begin
                    if (Debug) $display ("at time %t WRITE: Write Bank %d addr %d with Autoprecharge", $time, ba, addr);
                    Command[0] = `WRITE_A;
                end else begin
                    if (Debug) $display ("at time %t WRITE: Write Bank %d addr %d without Autoprecharge", $time, ba, addr);
                    Command[0] = `WRITE;
                end
                Col_addr[0] = addr[col_bits-1 : 0];
                Bank_addr[0] = ba;
    ...
    specify
        ...
        $setuphold(posedge Dq_chk, data_wr,    tDS,  tDH);
    endspecify
